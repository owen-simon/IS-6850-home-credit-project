...
numeric_corr <- train |>
  ...
  summarise(across(everything(), ~ cor(.x, train$TARGET, use = "complete.obs"))) |>
...
cat_vars <- train |> select(where(is.character)) |> names()

categorical_corr <- purrr::map_dfr(cat_vars, function(var) {
  df <- train |>
    dplyr::count(.data[[var]], TARGET) |>
    tidyr::pivot_wider(names_from = TARGET, values_from = n, values_fill = 0) |>
    dplyr::mutate(Diff = abs(`1` / (`0` + `1`) - `0` / (`0` + `1`)))

  tibble::tibble(Predictor = var, Max_Proportion_Diff = max(df$Diff, na.rm = TRUE))
}) |>
  dplyr::arrange(desc(Max_Proportion_Diff))
