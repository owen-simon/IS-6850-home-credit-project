...
Other variables included in this list such as `OWN_CAR_AGE` which indcates the age of the client's car could be NULL because the client doesn't own a car. However, variables such as `OCCUPATION_TYPE` do not indicate 
...
missing_summary <- combined |>
  select(
    -TARGET,
    -SK_ID_CURR,
    -all_of(building_level_variables)
  ) |>
  summarize_all(~ sum(is.na(.))) |>
  pivot_longer(
    cols = everything(),
...
gt(missing_summary) |>
  tab_header(
    title = "Missing Data Summary",
    subtitle = "Excluding Building-Level Variables"
  )
```

Other variables included in this list such as `OWN_CAR_AGE` which indcates the age of the client's car could be NULL because the client doesn't own a car. 
