...
## 4.2 Train/Validation Split for Model Card Validation

```{r}
set.seed(42)

train_index <- createDataPartition(train_full$TARGET, p = 0.8, list = FALSE)

train_model <- train_full[train_index, ]
val_model   <- train_full[-train_index, ]

# Convert target labels for caret
train_model$TARGET <- factor(train_model$TARGET,
                             levels = c(0,1),
                             labels = c("No","Yes"))

val_model$TARGET <- factor(val_model$TARGET,
                           levels = c(0,1),
                           labels = c("No","Yes"))
```

## 4.3 3-Fold CV With Supplementary Features
...
During initial model comparison, LASSO was fit using `cv.glmnet()` to evaluate cross-validated performance on different feature sets. For hyperparameter tuning and final model training, `caret::train()` with the glmnet method is used. This approach allows integration of resampling strategies (such as SMOTE, which synthetically oversamples the minority class), automated hyperparameter search, and structured evaluation of cross-validated AUC, making the workflow more flexible and reproducible.
...
The LASSO model trained with SMOTE achieved a slightly higher cross-validated AUC than the model trained without sampling during 3-fold cross-validation on the subsampled data; therefore, SMOTE was selected for the final model to address class imbalance.

## 6.1 Fit Final LASSO Model With Best Hyperparameters

```{r}
ctrl_final_smote <- trainControl(
  method = "cv",
  number = 5,
  sampling = "smote",
  classProbs = TRUE,
...
# View top 20 variables by absolute impact
head(coef_df, 20)
```
...
## 7.2 Generate Predictions on Validation Set for Model Card Use
```{r}
# Also generate predicted probabilities on the validation set for model card
val_probs <- predict(
  lasso_final, 
  newdata = val_model, 
  type = "prob")[, "Yes"]
...
The resulting submission file received a Kaggle area under the ROC curve of 0.73036 private and 0.74181 public. The private leaderboard is calculated with approximately 80% of the test data.

# 9. Data Export for Future Use

```{r}
# Final trained model
saveRDS(lasso_final, "lasso_final_model.rds")

# Validation data and probabilities for model card
saveRDS(val_model, "validation_data.rds")
saveRDS(val_probs, "validation_probs.rds")

# Train/validation split indices (reproducibility)
saveRDS(train_index, "train_index.rds")

# Feature names (to ensure new data can be processed correctly)
feature_names <- colnames(train_full)
saveRDS(feature_names, "feature_names.rds")

# Cross-validated predictions and AUCs
saveRDS(preds_lasso_full, "cv_preds_lasso_full.rds")
saveRDS(auc_lasso_full, "cv_auc_lasso_full.rds")

# Optional: baseline predictions and probabilities
saveRDS(baseline_predictions, "baseline_predictions.rds")
saveRDS(baseline_probs, "baseline_probs.rds")

# Extract coefficients for the selected lambda
coef_lasso <- coef(lasso_final$finalModel, s = lasso_final$bestTune$lambda)

# Convert to tidy data frame for reporting top features
coef_df <- tibble(
  Feature = rownames(coef_lasso),
  Coefficient = as.numeric(coef_lasso)
) |>
...
saveRDS(coef_df, "lasso_feature_importance.rds")
```